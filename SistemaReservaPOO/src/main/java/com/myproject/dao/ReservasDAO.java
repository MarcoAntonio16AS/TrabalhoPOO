...
public class ReservasDAO {
    ...
    //==========================================================================
    // Comando para verificar disponibilidade de recurso:
    // Neste caso precisamos ver os IDs de recursos que já foram ou não reservados

    public List<ReservasModel> buscarReservasPorRecurso(int id_recurso) {

        List<ReservasModel> reservas = new ArrayList<>();

        //Comando SQL para procurar recurso
        String sql = "SELECT * FROM reservas WHERE id_recurso = ?";
        ...
        try (
            ...
            //Condições e quais colunas devem ser puxadas de recursos para
            ...
            while (rs.next()) {
                ...
                reserva.setId_recurso(rs.getInt("id_recurso"));

                reserva.setInicio_reserva(
                        rs.getTimestamp("inicio_reserva")
                                .toLocalDateTime()
                );

                reserva.setFim_reserva(
                        rs.getTimestamp("fim_reserva")
                                .toLocalDateTime()
                );

                reserva.setTotal_reserva(
                        rs.getDouble("total_reserva")
                );

                reserva.setStatus_reserva(
                        rs.getString("status_reserva")
                );
            ...
            System.out.println(
                    "Erro ao buscar reservas do recurso: "
                            + e.getMessage()
            );

        }

        //Caso opere tudo bem, veremos a lista sendo retornada
        return reservas;
